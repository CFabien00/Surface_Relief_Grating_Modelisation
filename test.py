...
class GratingTest(unittest.TestCase):
    ...
    def test_list_of_gratings(self):
        grating_list = self.srg_3.get_list_of_grating(0)
    ...
    def test_get_stretched_surface(self):
        srg_surface = self.srg_3.get_stretched_surface(
            stretching=0, dimension=10, step=1)
        self.assertEqual(int(srg_surface[0][0]), 0)
        self.assertEqual(int(srg_surface[-1][0]), 8)

    def test_get_surface(self):
        grating_surface = self.grating.get_surface(dimension=10, step=0.1)
